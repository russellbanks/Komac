...
class MsixTests : FunSpec({
    context("calculate package family name") {
        test("identity name and full identity publisher") {
            ...
            Msix.getPackageFamilyName(identityName, identityPublisher) shouldBe "PackageName_31kpdnra495ry"
        }

        test("empty identity name and identity publisher") {
            val identityName = ""
            val identityPublisher = ""
            Msix.getPackageFamilyName(identityName, identityPublisher) shouldBe "_werc8gmrzge18"
        }
        ...
        test("identity name containing underscore and identity publisher with non-ASCII characters") {
            ...
            Msix.getPackageFamilyName(identityName, identityPublisher) shouldBe "my_app_name_0cddhv5z1eydp"
        }
        ...
        test("identity name and identity publisher containing only ASCII characters") {
            ...
            Msix.getPackageFamilyName(identityName, identityPublisher) shouldBe "AppName_zj75k085cmj1a"
        }

        test("identity name and identity publisher containing special characters") {
            val identityName = "My App-Name_1.0"
            val identityPublisher = "Publisher (Software)"
            Msix.getPackageFamilyName(identityName, identityPublisher) shouldBe "My App-Name_1.0_xz34dc2mhee0e"
        }

        test("identity name and identity publisher containing only non-alphanumeric characters") {
            val identityName = "!@#$%^&*()"
            val identityPublisher = "!@#$%^&*()"
            Msix.getPackageFamilyName(identityName, identityPublisher) shouldBe "!@#$%^&*()_3twee7s7mwzyy"
        }
        ...
        test("identity name and identity publisher containing only numeric characters") {
            ...
            Msix.getPackageFamilyName(identityName, identityPublisher) shouldBe "12345_pydnmasy62pxp"
        }

        test("identity name and identity publisher containing only whitespace characters") {
            val identityName = "   "
            val identityPublisher = "   "
            Msix.getPackageFamilyName(identityName, identityPublisher) shouldBe "   _2bkd433cae3jj"
        }

        test("identity publisher that is very long") {
            val identityName = "x"
            val identityPublisher = "y".repeat(1024)
            Msix.getPackageFamilyName(identityName, identityPublisher) shouldBe "x_r0rvk7mj3mn82"
        }
    }
})
