...
object GitHubImpl {
    ...
    val microsoftWinGetPkgs: GHRepository by lazy {
        ...
        while (result == null) {
            try {
                result = github.getRepository(WingetPkgs.FULL_NAME)
            } catch (ioException: IOException) {
                if (++count == maxTries) {
                    throw CliktError(
                        message = "Failed to get ${WingetPkgs.FULL_NAME}",
                        cause = ioException,
                        statusCode = 1
                    )
                }
    ...
    fun getWingetPkgsFork(terminal: Terminal): GHRepository = with(terminal) {
        ...
        while (result == null) {
            try {
                result = github.getRepository("$forkOwner/${WingetPkgs.WINGET_PKGS}")
            } catch (exception: GHFileNotFoundException) {
                info("Fork of ${WingetPkgs.WINGET_PKGS} not found. Forking...")
                try {
                    github.getRepository(WingetPkgs.FULL_NAME).fork().also {
                        success("Forked ${WingetPkgs.WINGET_PKGS} repository: ${it.fullName}")
                    }
                } catch (ioException: IOException) {
                    throw CliktError(
                        message = theme.danger("Failed to fork ${WingetPkgs.WINGET_PKGS}. Please try again or fork it manually"),
                        cause = ioException,
                        statusCode = 1
                    )
                }
                ...
                if (++count == maxTries) {
                    throw CliktError(
                        message = "Failed to get $forkOwner/${WingetPkgs.WINGET_PKGS}",
                        cause = ioException,
                        statusCode = 1
                    )
                }
