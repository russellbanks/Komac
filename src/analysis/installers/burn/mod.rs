...
use crate::{
    analysis::{
        ...
        installers::{
            PE,
            pe::resource::{ImageResourceDataEntry, ResourceDirectory, SectionReader},
        },
    },
    read::ReadBytesExt,
    traits::FromMachine,
...
const WIXBURN_HEADER: [u8; 8] = *b".wixburn";
...
impl Burn {
    pub fn new<R: Read + Seek>(mut reader: R) -> Result<Self, BurnError> {
        let pe = PE::read_from(&mut reader)?;

        let wixburn_section = pe
            .section_table
            .sections()
            .iter()
            .find(|section| section.raw_name() == WIXBURN_HEADER);

        let Some(wixburn) = wixburn_section else {
            // Get the resource table data directory header
            let resource_table = pe
                .optional_header
                .data_directories
                .resource_table()
