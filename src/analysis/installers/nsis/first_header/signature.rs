...
#[derive(Copy, Clone, PartialEq, Eq, TryFromBytes, Immutable)]
#[repr(C)]
pub struct NsisInstall(Sig1, Sig2, Sig3);

impl fmt::Debug for NsisInstall {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "NsisSignature({self})")
    }
}

impl fmt::Display for NsisInstall {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
...
mod tests {
    ...
    use super::NsisInstall;

    #[test]
    fn size() {
        assert_eq!(size_of::<NsisInstall>(), size_of::<u32>() * 3);
    }

    #[test]
    fn signature_from_bytes()
    -> Result<(), ValidityError<[u8; size_of::<NsisInstall>()], NsisInstall>> {
        // https://github.com/NSIS-Dev/nsis/blob/v311/Source/exehead/fileform.h#L229

        // Test the latest signature
        let buf: [u8; size_of::<NsisInstall>()] = transmute!([
            U32::<LE>::new(0x6C6C754E),
            U32::<LE>::new(0x74666F73),
            U32::<LE>::new(0x74736E49)
        ]);

        let nsis_signature: NsisInstall = try_transmute!(buf)?;

        assert_eq!(nsis_signature.to_string(), "NullsoftInst");
